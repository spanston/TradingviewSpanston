...
description: Perform Harmonic Elliott Wave analysis in TradingView using top-down wave counts, A-B-C motive internals, HEW ratio validation, alternation, projection maps, primary and alternate counts, invalidation, flip levels, and risk-defined posture. Use when the user asks for HEW, Harmonic Elliott Wave, Elliott Wave, wave count, wave 3 projection, C of 3, wave 5 target, Fibonacci projection, harmonic wave strategy, or ratio-driven structural forecasting of a symbol.
---
...
- Validate the count with ratios, support/invalidation, alternation, and degree logic. A count that looks clean but fails the ratio work is weak.
...
- Separate structural setup from execution setup. A valid map does not authorize a trade unless trigger, invalidation, reward, and alternate-count response are defined.
...
Check Wave 2 versus Wave 1 origin, B of 3 versus Wave 2, Wave 4 versus B of 3, and B of 5 versus Wave 4. Then test alternation: Wave 2 and Wave 4 should differ in depth, duration, or complexity. If they are too similar, the count is weaker.
...
- Wave 4 must respect B of 3, otherwise the five-wave count is damaged or invalid.
...
- Do not use trade language when the setup is only structural.
