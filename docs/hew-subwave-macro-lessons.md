...
- Lower-degree subwaves inside macro waves must be visually distinct from the macro count: use a lower TradingView Elliott degree, separate color/style, and a HEW Fibonacci projection zone when it clarifies the proof.
- Wave markers must come from TradingView native Elliott tools only: impulse, correction, triangle, double combo, or triple combo. Do not use text labels as wave markers.
- Macro/subwave overlap should be solved with native TradingView Elliott degree/style separation and chart scaling. Do not falsify evidence pivot prices to make labels look cleaner.
...
- The concrete style fixture is `fixtures/hew/good/team_no_clean_trade/input`; use `docs/hew-atlassian-reference-style.md` as the distilled version for future runs.
...
6. Drew the subwave count with a native TradingView Elliott impulse tool and added a HEW fib zone at `95.41-98.21`.
...
- Native TradingView Elliott-tool drawings for counts and subwaves.
...
- No text labels as wave markers.
